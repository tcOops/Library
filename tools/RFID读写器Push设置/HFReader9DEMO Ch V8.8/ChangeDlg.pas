...
var
 ...
 // aListItem:TListItem;
begin
  if frmHFReaderdemomain.ListView1.Selected = nil then
  begin
    MessageDlg('没有选择设备！', mtWarning, [mbOK], 0);
    exit;
  end;
  if frmHFReaderdemomain.ListView1.Selected.SubItems[0]<>''then
  begin
    cmpip:=frmHFReaderdemomain.ListView1.Selected.SubItems[1];
    cmdmac:= frmHFReaderdemomain.ListView1.Selected.SubItems[0];
    select_index:= frmHFReaderdemomain.ListView1.Selected.Index;
  end
  else
  begin
    Exit;
