...
function JsonOfBook(response) {
    ...
    var tmp = '<table style="border-width:0px;border-collapse:collapse;width:100%"><tr style="background-color:#E0E0E0;height:40px;border-width:0px">'
		...
		+  '<td style="width:14%; border-width:0px;font-weight:bold;text-align:center"> 标识码 </td>'
	    +  '<td style="width:20%; border-width:0px;font-weight:bold;text-align:center"> 书名</td>'
		+  '<td style="width:18%; border-width:0px;font-weight:bold;text-align:center">SN Code</td>'
        +  '<td style="width:17%;border-width:0px;font-weight:bold;text-align:center">RFID Code</td>'
    ...
    tmp += '<td style="width:14%; border-width:0px;text-align:center">' + book.Serial_code + '</td>'
    tmp += '<td style="width:20%; border-width:0px;text-align:center">' + book.Name + '</td>'
    tmp += '<td style="width:18%; border-width:0px;text-align:center">' + book.Sn_code + '</td>'
    tmp += '<td style="width:17%;border-width:0px;text-align:center">' + book.Signal_code + '</td>'
  //  tmp += '<td style="width:11%;border-width:0px;text-align:center">' + book.Status + '</td>'
    tmp += '<td style="width:10%;border-width:0px;text-align:center"> <button class="btn btn-sm" data-toggle="modal" data-target="#bookSign" onclick=' + '\'showBookInfo('
      + book.Id.toString() + ',\"' + book.Name + '\",\"' + book.Author + '\",\"' + book.Serial_code + '\",\"' + book.Signal_code + '\",\"' + book.Publisher_name
      + '\")\'>标识图书</button></td></tr></table>'
    $("#bookContent").html(tmp);
}


function showBookInfo(bookId, bookName, bookAuthor, bookSerialCode, bookSignalCode, bookPublisher) {
    url = "ajax.aspx"
    data = new Object()
